...
export function TrackerDialogsProvider({ children }: { children: ReactNode }) {
  ...
  const personRecords = records.filter((r) => !paymentForm.person_id || r.person_id === paymentForm.person_id);
  ...
  async function submitPayment(force = false) {
    ...
    if (paymentForm.transaction_type === "monthly_extra" && !force) {
      ...
      if (dup) {
        setConfirm({
          title: "Monthly extra already recorded for this period",
          description: `A monthly extra for ${period} was already recorded for this record. Do you want to record another payment for the same month?`,
          confirmLabel: "Yes, record it",
          onConfirm: () => {
            void submitPayment(true);
          },
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Monthly extra (₹)" hint="Kept fully separate from principal.">
              <Input
                ...
                onChange={(e) => setRecordForm({ ...recordForm, monthly_extra_amount: e.target.value })}
                placeholder="1000"
              />
            </Field>
            <Field label="Monthly extra start date">
              <Input
                ...
                onChange={(e) => setRecordForm({ ...recordForm, monthly_extra_start_date: e.target.value })}
              />
            </Field>
            <div className="sm:col-span-2">
              <Field label="Principal repayment condition">
                <Select
                  ...
                  onValueChange={(v) => setRecordForm({ ...recordForm, principal_repayment_condition: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            {recordForm.principal_repayment_condition === "specific_date" ? (
              <Field label="Repayment date *">
                <Input
                  ...
                  onChange={(e) => setRecordForm({ ...recordForm, principal_due_date: e.target.value })}
                />
              </Field>
            ) : null}
            <div className="sm:col-span-2">
      ...
      <Dialog open={paymentOpen} onOpenChange={setPaymentOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Monthly extra payments never reduce the principal.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <Field label="Payment type">
                <Select
                  ...
                  onValueChange={(v) => setPaymentForm({ ...paymentForm, transaction_type: v as TxnType })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <Field label="Person">
              <Select
                ...
                onValueChange={(v) => setPaymentForm({ ...paymentForm, person_id: v, money_record_id: "" })}
              >
                <SelectTrigger>
            ...
            {selectedSummary ? (
              <div className="rounded-xl bg-muted p-3 text-xs text-muted-foreground sm:col-span-2">
                Current values — Original principal <b className="text-foreground">{inr(selectedSummary.record.principal_amount)}</b>,
                remaining <b className="text-foreground">{inr(selectedSummary.remainingPrincipal)}</b>, extra
      ...
      <Dialog open={demandOpen} onOpenChange={setDemandOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {demandRecord ? `${personName(demandRecord.person_id)} · ${MONEY_TYPE_LABEL[demandRecord.type]}` : ""}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
